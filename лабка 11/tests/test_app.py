...
class TestClass:
    ...
    def test_user_list(self):
        ...
        resp = resp.json()
        assert resp[1]['email'] == 'taraskis06@gmail.com', f"Unexpected response:\n{resp}"
        assert resp[2]['email'] == 'dmitrokasprivskij@gmail.com', f"Unexpected response:\n{resp}"

    def test_user_add(self):
        headers = {'Content-Type': 'application/json'}
        data = {'email': "someone@mail.com"}
        resp = self._client.post(self._baseurl, headers=headers, json=data)
        assert resp.status_code == 201, f"Invalid response: {resp.status_code}"

        resp = self._client.get(self._baseurl)
        assert resp.status_code == 200, "Invalid response"
        resp = resp.json()
        assert resp[0]['email'] == 'taraskis06@gmail.com', f"Unexpected response:\n{resp}"
        assert resp[1]['email'] == 'dmitrokasprivskij@gmail.com', f"Unexpected response:\n{resp}"
